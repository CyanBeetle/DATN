"""Quick video testing - processes only first 10% of videos for rapid testing."""
import os
import sys
import cv2
import time
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(settings.LOGS_DIR, 'quick_test_processing.log'))
    ]
)
...
def show_videos_properties(video_files):
    ...
    root = tk.Tk()
    root.title("Quick Test Video Selection")
    root.geometry("700x550")
    
    # Add title and instructions
    tk.Label(
        root, 
        text="Traffic Monitoring Quick Test", 
        font=("Arial", 16, "bold")
    ).pack(pady=10)
    
    tk.Label(
        root, 
        text="Select videos to process (only first 10% will be processed)", 
        font=("Arial", 10)
    ).pack(pady=5)
    ...
    columns = ("Filename", "Resolution", "FPS", "Duration", "Total Frames", "Quick Test Frames")
    ...
    for i, props in enumerate(video_properties):
        # Calculate frames for quick test (10%)
        quick_test_frames = int(props['total_frames'] * 0.1)
...
def select_processing_fps(video_fps):
    ...
    tk.Label(
        ...
        text="Higher FPS provides better tracking accuracy but increases processing time.\n"
             ...
             "Note: Only the first 10% of each video will be processed.",
        justify="left",
        font=("Arial", 10)
    ).pack(pady=10)
    ...
    def update_frame_skip_preview():
        ...
        expected_time_var.set(f"Estimated processing time: {estimated_minutes:.1f} minutes per minute of video (10% only)")
...
def process_videos(input_folder, output_folder, roi_points, target_width, target_height, selected_videos, processing_fps):
    """
    Process videos in the input folder - only first 10% of each video
    ...
    try:
        ...
        print("\nPreparing to process videos (quick test mode - 10% only)...")
        ...
        for i, video_path in enumerate(selected_videos):
            filename = os.path.basename(video_path)
            output_path = os.path.join(output_folder, f"quick_test_{os.path.splitext(filename)[0]}.mp4")
            ...
            base_name = f"quick_test_{os.path.splitext(filename)[0]}"
            ...
            print(f"\nProcessing video {i+1}/{len(selected_videos)}: {filename} (first 10% only)")
            ...
            try:
                ...
                # Calculate max_duration to process only 10% of the video
                max_frames = int(total_frames * 0.1)  # 10% of total frames
                max_duration = max_frames / fps  # Duration in seconds
                
                cap.release()
                
                frame_skip = max(1, int(fps / processing_fps))
                print(f"  Frame skip: {frame_skip} (processing at approximately {processing_fps:.2f} FPS)")
                print(f"  Processing only first {max_duration:.1f} seconds ({max_frames} frames, 10% of video)")
                
                # Process the video with max_duration to limit to first 10%
                ...
                stats = processor.process_video(
                    ...
                    result_prefix="quicktest_"  # Add prefix for JSON results
                )
                
                # Update stats
                stats["input_file"] = filename
                stats["output_file"] = os.path.basename(output_path)
                stats["quick_test_mode"] = True
                stats["percentage_processed"] = 10
                total_vehicles += stats["unique_vehicles"]
                success_count += 1
                results.append(stats)
                
                print(f"  ✓ Success! Processed {stats['processed_frames']} frames (10% of video)")
        ...
        report_path = os.path.join(output_folder, f"quick_test_report_{timestamp}.json")
        ...
        roi_path = os.path.join(output_folder, "quick_test_roi_points.json")
        roi_data = {
            "roi_points": roi_points,
            "width_meters": target_width,
            "height_meters": target_height,
        ...
        print("\n" + "=" * 60)
        print(f"Quick Test processing completed in {processing_time:.1f} seconds")
        print(f"Videos processed: {success_count} successful, {error_count} failed")
        print(f"Total vehicles detected: {total_vehicles}")
        print(f"Processed at: {processing_fps:.2f} FPS (10% of each video)")
...
def main():
    """Main function to process traffic videos - Quick Test Mode (10% only)"""
    try:
        print("=" * 80)
        print(" Traffic Monitoring System - Quick Test Mode (10% of videos only) ")
        print("=" * 80)
        
        # Get input and output folders from config
        input_folder = settings.VIDEO_INPUT_DIR
        output_folder = settings.VIDEO_OUTPUT_DIR
        ...
        if action == "selected":
            ...
            print(f"\nSelected {len(selected_video_paths)} videos for quick testing.")
        else:  # action == "all"
            selected_video_paths = video_paths
            print(f"\nQuick testing all {len(selected_video_paths)} videos.")
        ...
        # Step 5: Process videos (10% only)
        print("\nStarting quick test processing (10% of each video)...")
        process_videos(
